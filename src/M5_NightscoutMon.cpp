  ...
  if (btnB_wasPressed)
  {
    ...
    */
    ...
    else
    {
      ...
      Serial.print("snoozeUntil = ");
      Serial.println(snoozeUntil);
    }
    ...
    Serial.print(tmpStr);
    Serial.print(", snoozeUntil-now = ");
    Serial.println(snoozeRemaining);
...
void wifi_connect()
{
  ...
  if (is_task_bootstrapping)
  {
    ...
    Serial.print("SSID Passphrase: ");
    Serial.println(ssid_passphrase);
    M5.Lcd.print("SSID Passphrase: ");
...
int readNightscout(char *url, char *token, struct NSinfo *ns)
{
  ...
  if ((WiFiMultiple.run() == WL_CONNECTED))
  {
    ...
    else
    {
      ...
      Serial.print("is_https_Heroku ");
      Serial.println(is_https_Heroku);
      if (cfg.sgv_only)
      {
        ...
        else
        {
          ...
          if (ns->is_Sugarmate == 0)
          {
            ...
            */
            ...
            { // Railway uses "trend" instead of "direction"
...
void handleAlarmsInfoLine(struct NSinfo *ns)
{
  ...
  Serial.println(" sec");
  Serial.print("Snooze time remaining = ");
  Serial.print(snoozeRemaining);
  Serial.print(" sec, Snooze until ");
  Serial.println(snoozeUntil);
  char tmpStr[10];
  M5.Lcd.setTextDatum(TL_DATUM);
  if (snoozeRemaining > 0)
...
void draw_page()
{
  ...
  case 2:
  {
    ...
    if (tmprc != float(0.01) && tmprc != float(0.02) && tmprc != float(0.03) && tmprc != float(0.04))
    { // not an error
    ...
    if (humid != float(0.01) && humid != float(0.02) && humid != float(0.03) && humid != float(0.04))
    { // not an error
  ...
  if (!is_task_bootstrapping)
  {
    ...
    if (millis() - msCount > 15000)
    {
      ...
      */
      ...
      Serial.print("cfg.LED_strip_mode = ");
